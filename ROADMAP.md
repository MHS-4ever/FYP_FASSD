...
## Current Status: Phase 3 → Transitioning to Phase 4
...
- ⚠️ **Models need retraining** with fixed code

---

## 🔄 Current Phase: Model Retraining (Required)

### Priority 1: Retrain Baseline Models

**Task:** Train models with corrected class weights

**Steps:**
1. Train clean feature model:
   ```bash
   python train_baseline.py \
     --manifest E:\FYP\data\features\features_manifest_labeled.csv \
     --epochs 10 \
     --batch_size 256 \
     --save E:\FYP\models_saved\baseline_cnn_fixed.pth
   ```

2. Train robust model (clean + augmented):
   ```bash
   python train_baseline.py \
     --manifest E:\FYP\data\features_merged\features_manifest_combined.csv \
     --epochs 10 \
     --batch_size 256 \
     --save E:\FYP\models_saved\baseline_cnn_robust_fixed.pth
   ```

**Expected Time:** 30-45 minutes per model
...
### Priority 2: Validate Fixed Models

**Task:** Confirm models work correctly

**Script:** `evaluate_baseline.py`

**Success Criteria:**
- Balanced accuracy on bonafide AND spoof
- EER significantly improved
- AUC > 0.90
- No bias towards either class

---

## 📋 Phase 4: Advanced Models & Features (Next)
...
|-------|------|-----------|--------|
| 3.1 | Retrain baseline models | 1-2 hours | ⏳ Pending |
| 3.2 | Validate fixed models | 30 min | ⏳ Pending |
| 4.1 | Train Log-Mel model | 1 hour | ❌ Not started |
...
### Required Outputs
1. ✅ Working codebase (90% complete)
2. ⏳ Trained models (pending retraining)
